%% simTransition_baseline.m
% Simulates UI shocks under baseline calibration, in PE; flex prices;
%  nominal rigidity + Taylor rule, nominal rigidity + constant real rate 
%  for 18 months, and nominal rigidity + constant nominal rate for 18 months, 
%  all given real wage rigidity = 1
...
%  ordered by pe, flex, sticky, sticky + fixed m, sticky + fixed M
epsSticky = [0.000001 0.000001 0.000001 0.00001 0.000001 0.000001];
HssCell = cell(1,5);
transParamsCell = cell(1,5);
suffixCell = {'_pe','','','_fixedr','_fixedi'};
    ...
    switch i
        ...
        case 3
            transParams = struct('psi',360,'adjResource',0,'epsilon',6,'phiInt',1/m-1,'phiPi',1.5,'phiC',1/12,'phiRho',0,'zlb',1,'fixedM',0,'fixedm',0,...
                ...
                'forceEnd',72000); 
        case 4
            transParams = struct('psi',360,'adjResource',0,'epsilon',6,'phiInt',1/m-1,'phiPi',1.5,'phiC',1/12,'phiRho',0,'zlb',1,'fixedM',0,'fixedm',18,...
                'iota',1,'znext_gT1',0,'rhoZnext_g',nan,'znext_gT2',nan,'fixt',nan,...
                'forceEnd',72000); 
        case 5            
...
parfor i=1:5
    ...
    try
        [~,~,~,~,~,~,~,...
            ...
            RCEName,strcat('output/trans_baseline',suffixCell{i}),invariantParams,varyingParams,...
            equityShares_e,equityShares_u,epsSticky,i,znextDiff,goodsDiff,...
            tDiff,mDiff,equityValDiff,dvacpostDiff,(1-1)*dwageDiff,cTransSS,[],HssCell{i},[],0,i==1,0);
    catch e
