%% simTransition_baseline_dbar.m
% Simulates UI shocks under alternative assumptions on magnitude of extension,
...
load('output/RCE_baseline_dbarbar22');
...
varyingParamsCell = cell(1,12);
suffixCell = cell(1,12);
for i=1:12
    thisVaryingParams = varyingParams;
    
    thisDbar = ceil(i/3)*3; % {3,6,9,12} month extension...
    thisMonth = mod(i-1,3)+1; % in month {1-12,1,12}
    switch thisMonth
        case 1
            thisVaryingParams.dbar(1:12) = thisVaryingParams.dbar(1:12)+thisDbar;
            thisSuffix = ['dbar_',num2str(thisDbar),'_fixedi'];
        case 2
            thisVaryingParams.dbar(1) = thisVaryingParams.dbar(1)+thisDbar;
            thisSuffix = ['dbar_',num2str(thisDbar),'_1','_fixedi'];
        case 3
            thisVaryingParams.dbar(12) = thisVaryingParams.dbar(12)+thisDbar;
            thisSuffix = ['dbar_',num2str(thisDbar),'_12','_fixedi'];
    end     
