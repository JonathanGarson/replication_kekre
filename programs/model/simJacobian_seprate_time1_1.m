%% simJacobian_seprate_time1.m
% Characterizes Jacobian starting from the `time 1' transitional dynamics 
%  characterized in simGR_seprate_time1
...
%  dwageDiff implied by computational error given baseline transitional
%  dynamics
load(strcat('output/GR_seprate_time1.mat'),'znextTrans','zsupplyTrans',...
    'cTrans','prodTrans','tTrans','this_t','mTrans','this_m',...
    'vacpostTrans','sfNewHiresTrans','wTrans','this_w',...
    'betasTrans','deltaBarTrans','dist2','this_theta','this_mu','MTrans');
...
betasTrans = betasTrans(:,2:end);
deltaBarTrans = deltaBarTrans(2:end);
this_m = this_m(2:end);
this_theta = this_theta(2:end);
this_t = this_t(2:end);
this_equityVal = dist2.initEquityVal;
this_w = this_w(:,2:end);
this_mu = this_mu(2:end);
this_M = MTrans(2:end);

clearvars znextTrans zsupplyTrans cTrans prodTrans tTrans ...
    mTrans equityValTrans equityVal vacpostTrans sfNewHiresTrans wTrans MTrans;

load(strcat('output/trans_baseline_noshocks_sticky.mat'),'cTrans');

load('output/RCE_baseline');

starting = struct('m',this_m,'theta',this_theta,...
    't',this_t,'equityVal',this_equityVal,...
    'w',this_w,'mu',this_mu,'M',this_M);
...
[equityShares] = ...
    ...
    starting.equityVal,dist2.z,[dist2.p_e_tilde1*dist2.phi_e_tilde1,(1-dist2.p_e_tilde1)*dist2.phi_u_tilde1]);
...
if varGroup == 1
    ...
    [thisM,thisT,~,thisEquityVal,thisW,~,~,...
        ...
        tDiff,mDiff,equityValDiff,dvacpostDiff,dwageDiff,cTrans(2:end),thisStarting,[],dist2,0,0,0); 
    ...
    H(5*Tbar-4+eStates*(Tbar-1)+1:6*Tbar-5+eStates*(Tbar-1),1) = (thisMTrans(1:Tbar-1)-starting.M(1:Tbar-1))'/0.0001;
	H(6*Tbar-5+eStates*(Tbar-1)+1:7*Tbar-6+eStates*(Tbar-1),1) = (thisCTrans(1:Tbar-1)-cTrans(2:Tbar))'/0.0001;
end
...
parfor i=1:poolobj.NumWorkers 
    ...
    for time=timeStart:timeEnd
        ...
        thisHpar((time-timeStart)*(7*Tbar-6+eStates*(Tbar-1))+5*Tbar-3+eStates*(Tbar-1):(time-timeStart)*(7*Tbar-6+eStates*(Tbar-1))+6*Tbar-5+eStates*(Tbar-1),1) = ...
            (thisMTrans(1:Tbar-1)-starting.M(1:Tbar-1))'/0.0001;
        thisHpar((time-timeStart)*(7*Tbar-6+eStates*(Tbar-1))+6*Tbar-4+eStates*(Tbar-1):(time-timeStart)*(7*Tbar-6+eStates*(Tbar-1))+7*Tbar-6+eStates*(Tbar-1),1) = ...
            (thisCTrans(1:Tbar-1)-cTrans(2:Tbar))'/0.0001;			
    end
    Hpar(:,i) = thisHpar;
end
...
save(['output/jacobian_seprate_time1_',num2str(varGroup)],'H');
