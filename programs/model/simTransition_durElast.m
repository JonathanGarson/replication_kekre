%% simTransition_durElast.m
% Simulates UI shocks under durElast calibration, in flex prices,
%  nominal rigidity + Taylor rule, and nominal rigidity + constant nominal
%  rate for 18 months, all given real wage rigidity = 1 
...
load(strcat('output/trans_durElast_noshocks_sticky.mat'),'znextTrans','zsupplyTrans',...
...
load('output/RCE_durElast');
load('output/jacobian_durElast_1.mat');
H1 = H;
clearvars H;
Tbar = 481;
...
% Define cells of H, eps, and transParams: 
...
parfor i=1:3
    ...
    try
        [~,~,~,~,~,~,~,...
            ...
            RCEName,strcat('output/trans_durElast',suffixCell{i}),invariantParams,varyingParams,...
            equityShares_e,equityShares_u,epsSticky,i,znextDiff,goodsDiff,...
            tDiff,mDiff,equityValDiff,dvacpostDiff,(1-1)*dwageDiff,cTransSS,[],HssCell{i},[],0,0,0);                   
    catch e
